...
public class SaveRecordService {
    ...
    public String saveTransaction(List<Record> records,String typeOfData){
        ...
        records.stream().forEach(record -> {
            try {
                ...
                String filename = ".\\"+typeOfData+"\\" + year + "\\" + quarter + "\\" + date + "-" + month + "-" + year + ".csv";
                ...
                if (checkFilePresent == true) {
                    log.info("Saving "+typeOfData+" Transaction starts with {} In present file ",record.getInit_date());
                    ...
                    log.info("Saving "+typeOfData+" Transaction starts with {} In new file",record.getInit_date());
                    CSVWriter writer = new CSVWriter(new FileWriter(file + "\\" + date + "-" + month + "-" + year + ".csv", true));
        ...
        if(errorIn.size()>0) {
            log.error("Problem with "+typeOfData+" index value : " + Arrays.toString(errorIn.stream().toArray()));
